"""Stage 3b 真模型用例：打本地 ollama。

只留两条：一条验异步分发真的在异步（慢观测者追不上），一条回归——
换了传输层之后，stage03 的中断语义没被改坏。
其余传输层行为都在 test_stage03b_transport.py 里离线验过了。
...
from baby_event_driven_agent.stages.stage03b_message_bus.agent import (
    INTERRUPTED,
    STOP_CLOSER,
    Agent,
)
...
from baby_event_driven_agent.stages.stage03b_message_bus.llm import RealLLM
from baby_event_driven_agent.stages.stage03b_message_bus.outbound import StreamConsumer
from baby_event_driven_agent.stages.stage03b_message_bus.subscribers import counter
...
@pytest.fixture()
def workdir() -> Path:
    d = Path(tempfile.mkdtemp(prefix="stage03b_live_"))
    yield d
...
def test_interrupt_semantics_survive_the_transport_rewrite(workdir: Path) -> None:
    ...
    assert hist[-1]["content"] in (INTERRUPTED, STOP_CLOSER), hist[-1]
