...
中断与转向的逻辑随 stage03 的语义统一同步更新：收尾一律补 assistant 占位
（文本按尾部角色选），打断可附一条新消息（turn 不结束）；没收到完整的 LLM
返回就当没收到，在飞 step 的产物一律丢。
"""
...
class Agent:
        ...
        for _ in range(MAX_STEPS):
            ...
            if pending is not None:
                if pending.get("text"):
                    ...
                    continue
                await self._close_stop(sid, history)
                await self._mark_boundary(sid)
    ...
    async def _close_stop(self, sid: str, history: list[dict[str, Any]]) -> None:
