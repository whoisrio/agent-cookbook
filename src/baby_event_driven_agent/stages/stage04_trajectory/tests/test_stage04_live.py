"""Stage 04 真模型用例：打 .env 配的模型（本地 ollama 也能跑）。

只留两条：
1. 一轮真对话后，轨迹落在盘上且投影合法（轨迹层的主路径）；
2. 回归——history 换成轨迹层之后，stage03 的中断语义没被改坏，
   且合成消息进的是轨迹（不只是事件流）。
其余都在 test_stage04_trajectory / test_stage04_agent / test_stage04_session
里离线验过了。
...
class Harness:
    ...
    def interrupt(self, text: str | None = None) -> None:
        ...
        self.bus.publish(
            Event("user_interrupt", self.sid, payload), to=self.agent.agent_id
        )
...
def test_turn_lands_on_disk_and_projects_legally(workdir: Path) -> None:
    ...
    from baby_event_driven_agent.stages.stage04_trajectory.session.trajectory import Trajectory, TrajectoryLog
...
def test_interrupt_semantics_survive_the_trajectory_rewrite(workdir: Path) -> None:
    ...
    assert last is not None and last["content"] in (INTERRUPTED, STOP_CLOSER)
