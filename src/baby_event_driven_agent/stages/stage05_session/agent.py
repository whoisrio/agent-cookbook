...
过的会话，`attach(traj)` 之后收工。中断 / steering / redirect 的逻辑一字未动：
...
# 中断收尾用的标记与占位（同 stage03/04，自描述是硬要求）
...
class Agent:
    ...
    async def _run_steps(self, sid: str, traj: Trajectory) -> None:
        ...
        for _ in range(MAX_STEPS):
            ...
            try:
                ...
                if await self._close_after_cancel(sid, traj, pending, partial):
                    continue
                return
            finally:
    ...
    async def _close_after_cancel(
        ...
        if pending.get("intent") == "redirect" and text:
            ...
            if calls:
                await self._append_synth(
                    sid, traj, self._partial_assistant(partial), "interrupted"
                )
                for call in traj.last_message()["tool_calls"]:  # type: ignore[index]
                    await self._append_synth(
                        sid,
                        traj,
            ...
            return True
        await self._append_synth(
            sid, traj, {"role": "user", "content": STOP_MARKER}, "marker"
        )
        await self._end_turn(sid, "interrupted")
        return False
