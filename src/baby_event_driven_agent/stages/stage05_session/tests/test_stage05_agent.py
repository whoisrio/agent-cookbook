"""Stage 5a agent 集成用例：全部离线（ScriptedLLM）。

要验的是"history 换成轨迹层之后，stage02-04 的行为一个没坏，且轨迹账目正确"：

- 一轮 turn 在轨迹里是合法序列：user → assistant(tool_calls) → tool → assistant
- 投影是每次 step 现算的（system 前置、历史完整、合成注脚被 strip）
- steering / 中断 / redirect 落到轨迹上的是合法序列 + synthetic 留痕
...
@pytest.fixture()
def workdir() -> Path:
    d = Path(tempfile.mkdtemp(prefix="stage05_agent_"))
    yield d
...
def test_interrupt_stop_marks_trajectory(workdir: Path) -> None:
    ...
    async def go() -> None:
        ...
        h.publish(Event("user_interrupt", h.sid, {"intent": "stop"}))
        await h.wait_turn()
    ...
    # 被掐的 step 不留半截：最后一条 user 是中断标记
    last = h.traj.last_message()
    assert last is not None and last["content"] == STOP_MARKER


def test_interrupt_redirect_keeps_turn_alive(workdir: Path, inventory_spy: list[dict[str, Any]]) -> None:
    """redirect：掐掉在飞的一步 + 补消息，turn 不结束，同一 turn 内继续。

    时序是钉死的：tool_call chunk 已消费（tool_call_started 已发、partial 里有
    tool_calls），流挂在 hang 上；此刻 redirect → 取消必然命中 → 占位补齐后
    同一 turn 内继续，最终回答收尾。
    """
    ...
    async def go() -> None:
        ...
        h.publish(
            Event(
                "user_interrupt",
                h.sid,
                {"intent": "redirect", "text": "不对，改成 45 件，规格也要改"},
            )
        )
    ...
    assert notes == ["interrupted", "tool skipped", "redirect"]
    # 被掐的那一步已累积出 tool_calls：补占位 assistant + NO_EXEC 占位结果
    tool_contents = [
        str(e.payload["message"].get("content"))
        for e in h.traj.entries()
        if e.type == "message" and e.payload["message"].get("role") == "tool"
    ]
    assert tool_contents and tool_contents[0] == NO_EXEC
    ...
    redirect_user = [
        ...
        and str(e.payload["message"]["content"]).startswith(REDIRECT_NOTE)
    ]
    assert redirect_user, "纠正消息要带着上下文标注进轨迹"
