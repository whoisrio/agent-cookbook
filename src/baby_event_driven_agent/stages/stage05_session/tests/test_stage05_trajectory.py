...
- 投影：路径遍历 + 类型分派 + sanitize；确定性（同文件同 policy 逐字节相同）；
  model_change 覆盖式提取；元数据跳过
- 压缩口子：first_kept 之前的跳过、摘要插在最前；rewind 到压缩之前旧消息原样回来
...
POLICY = ProjectionPolicy(system_prompt="SYS")
...
def new_traj(workdir: Path, name: str = "t.jsonl") -> Trajectory:
    return Trajectory.create(TrajectoryLog(workdir / name), sid="s" * 32)
...
def test_projection_dispatch_and_determinism(workdir: Path) -> None:
    ...
    p1 = traj.build_context(POLICY)
    p2 = traj.build_context(POLICY)
...
def test_projection_default_model_falls_back(workdir: Path) -> None:
    """路径上没有 model_change：model 回落 default（pi：null 由调用方兜底）。"""
    traj = new_traj(workdir)
    traj.append(MODEL_CHANGE, {"model_id": "m1"})
    u = user(traj, "问")
    assistant(traj, "答1")
    ...
    assert traj.build_context(POLICY).model == "m2"


def test_sanitize_dangling_tool_calls_two_tiers(workdir: Path) -> None:
    """悬挂的工具调用：placeholder 补自描述占位；drop 连 assistant 一起撤。

    两个档位都只改投影——原文件字节必须一个不变（修复不写回事实层）。
    """
    traj = new_traj(workdir)
    user(traj, "改库存")
    ...
    fixed = traj.build_context(POLICY)  # 默认 placeholder
    ...
    dropped = traj.build_context(ProjectionPolicy(system_prompt="SYS", repair="drop"))
    assert not any(m.get("tool_calls") for m in dropped.messages)
    assert dropped.stats["repaired"] == 1
    assert traj.log.raw_bytes() == before  # 两次修复都没碰文件
...
def test_compaction_skips_before_first_kept(workdir: Path) -> None:
    ...
    u2 = user(traj, "u2")
    assistant(traj, "a3")
    traj.append(COMPACTION, {"summary": "之前查过库存", "first_kept_id": u2.id})
    user(traj, "u3")
...
def test_rewind_before_compaction_restores_old_messages(workdir: Path) -> None:
    ...
    assistant(traj, "a1")
    comp = traj.append(
        COMPACTION, {"summary": "摘要", "first_kept_id": u.id}
    )
    user(traj, "u2")
    assert "<summary>" in json.dumps(traj.build_context(POLICY).messages)
...
def test_rewind_moves_pointer_only(workdir: Path) -> None:
    ...
    assert traj.build_context(POLICY).messages[-1]["content"] == "u1"
...
def test_append_after_rewind_creates_branch(workdir: Path) -> None:
    ...
    assert traj.build_context(POLICY).messages[-1]["content"] == "u2"
...
def test_branch_summary_is_a_view_not_dialogue(workdir: Path) -> None:
    ...
    msgs = traj.build_context(POLICY).messages
...
def test_fork_produces_independent_session(workdir: Path) -> None:
    ...
    assert reloaded.build_context(POLICY).messages[-1]["content"] == "a1"

    traj.append(MESSAGE, message_payload({"role": "user", "content": "旧会话继续"}))
    forked.append(MESSAGE, message_payload({"role": "user", "content": "新会话继续"}))
    assert traj.build_context(POLICY).messages[-1]["content"] == "旧会话继续"
    assert forked.build_context(POLICY).messages[-1]["content"] == "新会话继续"
...
def test_projection_is_pure_function_of_file(workdir: Path) -> None:
    """f(文件, policy) 的另一面：两个 Trajectory 实例读同一份文件，投影相同。"""
    traj = new_traj(workdir)
    user(traj, "u1")
    assistant(traj, "a1")
    twin = Trajectory.load(TrajectoryLog(traj.log.path))
    assert json.dumps(traj.build_context(POLICY).messages) == json.dumps(
        twin.build_context(POLICY).messages
    )
