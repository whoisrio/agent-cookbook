...
> 配套代码（初稿，代码尚未落地）：计划新增
> `src/baby_event_driven_agent/stages/stage05b_compaction/`（从 stage05_session
> 拷贝后增量改），`stage05b-demo` 跑演示、`stage05b-test` 跑测试；测试条数与
> 实测输出落地后回填，本稿不预填数字。
>
...
5a 立住了会话：resume、rewind、fork，轨迹是唯一事实，messages 是轨迹的
投影。但投影每次把当前路径全量交给模型，会话一长就装不下。5a 留了
compaction entry 和投影语义的口子，本章把扳机装上。

## 为什么要做压缩

模型的上下文窗口是有限的，而且这个限制不会因为窗口变大而消失。即便
现在的模型已经支持百万级上下文（GPT-1.1 的 extra 模式支持 1.1M），
长程任务下照样不够用：agent 连着干几个小时、跨几天 resume，工具结果
一轮轮堆上去，百万 token 也有见底的时候。
...
一种是按轮次：数 turn 数，超过 N 轮就压，token 都不用算。问题是每轮
体量方差很大，方差主要来自 tool_result——一轮查询返回几千行，顶二十
轮闲聊。同样 20 轮，带大结果的会话来不及压、直接超限，闲聊会话被白白
压掉一段好上下文。轮次是 token 的粗糙代理，每轮体量均匀才准，agent
会话不满足这个前提。

另一种是按 token 消耗：算上下文占了多少 token，超过窗口的某个比例就
压，和窗口限制直接对齐。但它只管住了什么时候压，下刀时还有问题：
tool_calls 和 tool 结果必须成对出现，从一对配对中间切开，保留段就是
非法序列，provider 会拒。按 token 找刀口，还得再写一层逻辑把刀口对齐
回回合边界。
...
窗口随对话往前滑：窗口里永远是最近 N 轮原文，窗口外被摘要替代；再压
时，旧摘要和新划出去的轮次一起压成新摘要（见"滚动折叠"）。
...
新包 `stage05b_compaction/` 从 `stage05_session/` 拷贝，
...
- 回归：stage05_session 全部既有用例在新包通过。
