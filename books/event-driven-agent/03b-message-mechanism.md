# Stage 4：能力优化: 让用户选择下一条消息的处理方式 | UI 缓冲 | 工具审批

> 配套代码：`src/baby_event_driven_agent/stages/stage03b_message_bus/`，
> `stage03b-demo` 跑演示（01~03、05 打真实模型：本地 ollama，读仓库根 `.env`；
> 04 只测总线，不打模型），
> `stage03b-test` 跑测试（24 条离线 + 2 条真模型，共 26 条）。
...
而hermes agent，默认行为则是interrupt；

这一章，咱们做如下优化，
第一，我们把咱们baby agent的消息机制按照codex/workbuddy的方式来处理；
另外，这一章，我们顺手处理如下两个问题，
1.在agent的事件消费端，LLM和thinking的stream输出每次有delta内容到来都会发送事件，现在的 flash 模型每秒能吐几百 token，stream 模式下 UI 上的呈现不能还是来一个 token 就刷一次；这一类事件的消费，需要设置接收缓冲区，到达一定数量之后，统一刷新一次；
因此，咱们也给UI消费添加上缓冲区，
2.我们给工具添加上审批消息的机制；
...
01~03、05 的回答由真实模型生成（本地 ollama，配置读仓库根 `.env`）；04 只测总线，
不打模型。case 可单独跑：

```
stage03b-demo                              # 跑全部（默认打真模型）
stage03b-demo 02-promote-to-steering       # 只跑一个 case
stage03b-demo --list                       # 列出所有 case
```

> 01~03、05 都以"模型真的发起一次工具调用"为前提，而小模型不保证每次都调工具
> （有时把工具调用写成文本、或直接反问）。demo 不做兜底：`open_tool_window()`
> 只等一次，拿不到"工具在飞"的窗口就打印说明并结束这一段。



### 01-followup-default：turn 在飞时的新消息默认 followup

![01-followup-default：turn 在飞时的新消息默认 followup，排队不插话](../../src/baby_event_driven_agent/rec/stage03b/docs/01-followup-default.gif)
...
![02-promote-to-steering：promote 把排队的 followup 消息升级成 steering 插话，本轮不断](../../src/baby_event_driven_agent/rec/stage03b/docs/02-promote-to-steering.gif)
...
![03-interrupt-redirect：user_interrupt 旁路带着纠正文本打断，本轮转向、纠正先落地](../../src/baby_event_driven_agent/rec/stage03b/docs/03-interrupt-redirect.gif)
...
![04-ui-coalescing-buffer：200 个 delta 合并成 3 帧刷屏，一条不丢](../../src/baby_event_driven_agent/rec/stage03b/docs/04-ui-coalescing-buffer.gif)
...
![05-approval-flow：工具执行前发 approval_required，人批准后工具才执行](../../src/baby_event_driven_agent/rec/stage03b/docs/05-approval-flow.gif)
