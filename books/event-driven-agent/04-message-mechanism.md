...
总线不知道谁要合并，也不该替每个消费者决定帧长。顺带把"没丢"的功劳记对地方：
那是 flood 里每 10 条一次 `sleep(0)` 挣来的（emit 自己不让出，见 `flood` 的
docstring）——`test_flood_without_yields_drops_beyond_queue_size` 钉死了这个
反事实：不让出时 736 条全丢，快消费者无能为力。
...
- **seq**：全局单调，**落盘即编号**（`EventLog.append` 分配），进程重启接着走。
  Stage 5a 拿它当回放位点、5b 拿它定压缩区间，Stage 6 拿它切 eval 切片。
  两个精确化，读 log 的人要知道：
  - **seq = 落盘顺序，不是 emit 的调用顺序**。emit 先 await 治理，有匹配的
    拦截者时会真让出，此窗口内别的 session 的事件会先拿号——全局 log 里
    seq 与 ts（构造时刻）可能逆序。**回放排序一律用 seq，不用 ts**。
  - **会话内因果不破**：同一 session 只有一个 worker，turn 内的 emit 全部
    await 串行，同一 session 不可能同时有两个 emit 在飞。按 session 过滤后，
    seq 就是事件发生顺序。这条由 `test_seq_is_persist_order_not_call_order`
    钉死，不是口头保证。
...
本章的落盘只解决"事件**能**留下来、能定位、能续读"（传输层）。至于留下来
之后怎么读——投影、恢复、压缩，log 的**语义**是下一章（Stage 5a/5b）的事：
残尾判定在这里是工程，"读到残尾意味着什么"在那里才有定义。
...
- **inbox 无界**：下行收件箱是 `asyncio.Queue()`，没有 maxsize——worker 卡住时
  用户消息无限堆积。下行低频，暂可接受；上界与满了的语义（拒绝还是落盘）
  和"走出单机"一起做。
...
Stage 4 保证了事件**能到达、能定位、能治理、能留下来**。Stage 5a（会话与真相）
接着解决另一半：多轮对话的状态怎么管——以及崩溃之后怎么从 log 里恢复。

- `history` 是内存里的工作态，进程一重启就没了——Stage 5a 把它换成会话存储，
  `seq` 正好就是轨迹坐标（回放位点、恢复边界都靠它）；
- `correlation_id` 已经把一次 turn 的事件聚成一簇，Stage 5a 要的是把这些簇
  串成一条**可回放**的轨迹；5b 再谈**可裁剪、可压缩**；
